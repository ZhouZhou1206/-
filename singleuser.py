...
# DataBase in Pytorch
   # dataset = Data.TensorDataset(data_tensor =  train_data, target_tensor = train_labels) #这句话出现了报错，是版本的问题，把句子改一下就好了
    dataset = Data.TensorDataset(train_data,train_labels)
    #dataset = Data.TensorDataset(data_tensor =  train_data, target_tensor = train_labels) #这句话出现了报错，是版本的问题，把句子改一下就好了
    dataset = Data.TensorDataset(train_data,train_labels)
    #datasettest = Data.TensorDataset(data_tensor =  test_data, target_tensor = test_labels) ##这句话出现了报错，是版本的问题，把句子改一下就好了 
...
#Training
    for epoch in range(NUM_EPOCHS):
       for step, (x, y) in enumerate(train_loader):
        ...
        if step % 100 == 0:
           # print('Epoch: ', epoch, '| train loss: %.4f' % loss.data[0]) ##这句话出现了报错，是版本的问题，把句子改一下就好了
